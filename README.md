...
## Архитектура бэкдора. SSH

Выше уже было рассмотрено понятие: сетевая архитектура клиент-сервер. Для реализации бэкдора был использован следующий принцип архитектуры: сервер посылает команды на компьютер с бэкдором через SSH канал, а программа клиент их выполняет и возвращает ответы в виде сообщения, файлов.
...
2) Хост Б отправляет SYNchronize-ACKnowledgement.
Хост А получает SYN-ACK от Б;
...
После этого нужно сервер ждет запроса соединения с компьютера жертвы. Как только сервер получил сокет жертвы:
...
```
- запускается сервер
- извлекается канал, открытый клиентом через объект параметров безопасности, и если был получен канал, происходит небольшая коммуникация.

Если объект параметров транспорта вернет `None`, значит пользователь не прошел аутентификацию или было превышено время ожидания открытого канала со стороны клиента.
...
Сервер заканчивает свою работу по команде `server stop`, которая вызывает у объекта параметров защищенного соединения метод `close`, закрыващий сессию и все привязанные к ней каналы.
...
- Отказано в доступе из-за неверного пароля/имени пользователя  
...
Теперь все готово, чтобы использовать основной метод `open_door`, который представляет собой бесконечный цикл: получение команды - её обработка - исполнение - возвращение результата. Выйти из него можно только с помощью специальной команды со стороны сервера: `server stop`.
